...
namespace ConsoleApp2
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var xenonautsInstallDirectory = settings.XenonautsInstallDirectory.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar;
            var modsDirectory = settings.ModsDirectory.TrimEnd(Path.DirectorySeparatorChar); 

           
            if (!Directory.Exists(xenonautsInstallDirectory))
            {
                Console.WriteLine("Could not find Xenonauts 2 install directory");
                Console.WriteLine("");
            ...
            {
                Console.WriteLine("Could not find Mods directory");
                Console.WriteLine("");
                Console.WriteLine("Press any key to continue.");
                Console.ReadLine();
                return;
            ...
            var hasLoadMods = module.GetType("Xenonauts.XenonautsMain").GetMethods().Where(x => x.FullName.Contains("LoadMods")).Any();
            ...
            if (!hasLoadMods)
            {
                ...
                Console.WriteLine("Dll patched, opening game");
            }
            else
            {
            ...
            Directory.SetCurrentDirectory(@$"{xenonautsInstallDirectory}\");
            Process.Start("CMD.exe", "/c Xenonauts2.exe");
